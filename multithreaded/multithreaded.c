...
int main (int argc, char *argv[]) {
	...
	video_convert = gst_element_factory_make("ffmpegcolorspace", "csp"); //videoconvert in gst1.0
	video_sink = gst_element_factory_make("autovideosink", "video_sink");

	// Create the empty pipeline
	pipeline = gst_pipeline_new("test-pipeline");
        
        if (!pipeline || !audio_source || !tee || !audio_convert  || !audio_resample || !audio_sink || !video_queue || !visual || !video_convert || !video_sink) {
